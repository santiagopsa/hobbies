...
# Momentum exhaustion heuristics
...
COOLDOWN_MIN_AFTER_LOSS = 20  # minutes (block re-entries for this symbol)
...
def hybrid_decision(symbol: str):
    ...
    # ==== NEW: per-symbol cooldown after a recent recorded loss
    try:
        ...
        if info and info.get("ts"):
            ...
            if last_dt and (now - last_dt).total_seconds() < COOLDOWN_MIN_AFTER_LOSS * 60:
                return "hold", 55, 1.0, "cooldown after loss"
    except Exception:
        pass

    # ==== NEW: post-loss re-entry guard until 15m momentum resets
    info = LAST_LOSS_INFO.get(symbol)
    if info and tf15:
        macdh_15 = tf15.get('MACDh', 0)
        last_15 = tf15.get('last'); ema20_15 = tf15.get('EMA20')
        if (macdh_15 is not None and macdh_15 <= 0) or (last_15 is not None and ema20_15 is not None and last_15 < ema20_15):
    ...
    if score >= score_gate:
        ...
        # ==== NEW: cap confidence after recorded loss
...
def dynamic_trailing_stop(symbol: str, amount: float, purchase_price: float, trade_id: str, atr_abs: float):
    ...
    price-based: trail% (scaled by ATR) and optional TP
    indicator-based: exit if 15m RSI>70 & MACDh turns negative, or ADX1h collapses, etc.
    """
    def loop():
        try:
                ...
                if EXIT_MODE in ("indicators", "hybrid"):
                    ...
                    try:
                        if df15 is not None and df1h is not None:
                            ...
                            # exhaustion rules (tune as needed)
                            ...
                            if indicator_exit:
                                logger.info(f"[trail-exit] {symbol} momentum agotado (RSI15={rsi15}, MACDh15={macdh15}, ADX1h={adx1h})")
                    except Exception as e:
...
def analyze_and_learn(trade_id: str, sell_price: float = None):
    try:
        ...
        # ==== NEW: remember losses for cooldown/sizing
        try:
            if pnl_usdt <= 0:
...
def size_position(price: float, usdt_balance: float, confidence: int, symbol: str = None) -> float:
    ...
    # ==== NEW: dampen sizing for 2h after last loss on this symbol
...
def trade_once_with_report(symbol: str):
    ...
    try:
        ...
        # ==== NEW: quick early cooldown (defensive)
        try:
            now = datetime.now(timezone.utc)
            info = LAST_LOSS_INFO.get(symbol)
            if info and info.get("ts"):
                last_dt = datetime.fromisoformat(info["ts"].replace("Z",""))
                if last_dt and (now - last_dt).total_seconds() < COOLDOWN_MIN_AFTER_LOSS * 60:
                    report["note"] = f"cooldown after loss ({symbol})"; logger.info(f"{symbol}: {report['note']}")
                    log_decision(symbol, 0.0, "hold", False); return report
        except Exception:
            pass
