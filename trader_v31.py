...
# >>> ML LIGHT (Simple scikit-learn, no heavy deps like FreqAI)
try:
...
MAX_OPEN_TRADES = int(os.getenv("MAX_OPEN_TRADES", "2"))  # v31: reduced from 5→2 (correlated positions = same bet)
RESERVE_USDT = 20.0
...
_EQUITY_MONTHLY_BREAKER_ACTIVE = False  # Global flag: monthly DD breaker active
_EQUITY_MONTHLY_BREAKER_UNTIL = None  # Date until which monthly breaker is active
_EQUITY_MONTHLY_START_BALANCE = None  # Starting balance for monthly calculation

# >>> FGI CONTROL (Minimize FGI - make optional)
FORCE_FGI = bool(int(os.getenv("FORCE_FGI", "0")))  # If 0, FGI gate is disabled (redundant with price/RSI)
...
# >>> BACKTESTING BUILT-IN (Simple, faster than Freqtrade CLI)
BACKTEST_ENABLED = bool(int(os.getenv("BACKTEST_ENABLED", "0")))  # Run backtest mode
BACKTEST_START = os.getenv("BACKTEST_START", "2020-01-01")
BACKTEST_END = os.getenv("BACKTEST_END", "2025-11-21")
BACKTEST_TIMEFRAME = os.getenv("BACKTEST_TIMEFRAME", "1h")

# >>> PARAM OPTIMIZATION (Grid search, simpler than Freqtrade Optuna)
OPTIMIZE_ENABLED = bool(int(os.getenv("OPTIMIZE_ENABLED", "0")))
OPT_PARAMS = {
    'rsi_low': [30, 35, 40],  # Tune for DCA
    'adx_min': [15, 20, 25],  # For swing
    'vol_slope_min': [0, 50, 100]  # JSON edge
}

# >>> ML LIGHT (Simple scikit-learn, no heavy deps like FreqAI)
ML_ENABLED = bool(int(os.getenv("ML_ENABLED", "0")))
ML_MODEL = None  # Will be trained on historical trades
...
SCORE_GATE_HARD_MIN = float(os.getenv("SCORE_GATE_HARD_MIN", "50.0"))  # v31: raised from 35→50 (fewer trades, better quality)
...
# Buy-flow controller
TARGET_BUY_RATIO = 0.25  # v31: reduced from 0.38→0.25 (less aggressive starvation relief)
...
MIN_HOLD_SECONDS = int(os.getenv("MIN_HOLD_SECONDS", "14400"))  # v28-fix: 4h minimum hold for swing (was 2h — too short, most winners need >4h)
...
PARTIAL_SELL_COOLDOWN_SEC = int(os.getenv("PARTIAL_SELL_COOLDOWN_SEC", "1800"))  # 30 min
MAX_PARTIAL_SELLS_PER_TRADE = int(os.getenv("MAX_PARTIAL_SELLS_PER_TRADE", "2"))
# >>> PATCH END
...
# >>> PATCH END
# >>> PARK GUARD STATE (ANCHOR PKF_STATE) - Now using _TRADE_STATES dict
# PARK_GUARD replaced with _TRADE_STATES["PARK_BTC"] for parking (BTC-specific virtual trade)
PARK_TRADE_ID = "PARK_BTC"  # Fixed ID for parking virtual trade
...
def initialize_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS equity_guard_state (
...
def fetch_fgi_features() -> dict:
    ...
    try:
        ...
        try:
            ...
            fallback_value = fgi_data.get("value")
        except Exception:
            fallback_value = None
        
        # Default to neutral (50) if all fails - don't block trades
        if fallback_value is None:
            fallback_value = 50  # Neutral default
            logger.debug("FGI fetch failed, using default neutral value 50")
...
def volume_snapshot(symbol: str) -> dict:
    ...
    return {
        ...
        "adx1h":  f1h.get("ADX"),
        "rsi1h":  f1h.get("RSI"),  # Added for decay check in bears
    }
...
def hold_in_volume(symbol: str) -> dict:
    ...
    # >>> PROXY INTEGRATION: Disable decay if proxy active (RSI4h < RSI_FEAR_PROXY)
    # Volume dries up in bottoms but rebounds come, so don't exit on decay in fear
    proxy_active_decay = False
    try:
        rsi4h_proxy_decay = get_btc_rsi4h_cached()  # Use cached helper
        if rsi4h_proxy_decay is not None and rsi4h_proxy_decay < RSI_FEAR_PROXY:
                proxy_active_decay = True
                logger.debug(f"Proxy active: disabling decay check (RSI4h={rsi4h_proxy_decay:.1f}<{RSI_FEAR_PROXY})")
    except Exception:
        pass
    
    rsi1h_for_decay = vs.get("rsi1h") or 0.0
    ...
    try:
        rsi4h_btc_decay = get_btc_rsi4h_cached()
...
def park_to_btc_if_needed() -> bool:
    ...
    Requiere:
      PARK_PROTECT_ENABLED, PARK_TRAIL_BPS, PARK_REENTRY_PAD_BPS,
    ...
    if not park_state:
        park_state = {"anchor": None, "high": None, "last_change": 0, "floor": None, "reason": None, "parked": False, "symbol": "BTC/USDT"}
        ...
        with PARK_LOCK:
            if PARK_TRADE_ID not in _TRADE_STATES:
                _TRADE_STATES[PARK_TRADE_ID] = {
                    ...
                    "high": None,
                    "floor": None,
                    "last_change": None,  # epoch
                    "reason": None,
                    "parked": False,
                    "symbol": "BTC/USDT"
                }
            park_state = _TRADE_STATES[PARK_TRADE_ID]
            park_state.setdefault("anchor", None)
            park_state.setdefault("high", None)
            park_state.setdefault("floor", None)
            park_state.setdefault("last_change", None)  # epoch
            park_state.setdefault("reason", None)
            park_state.setdefault("parked", False)
    ...
    def _update_anchor_and_floor(last_px: float):
        ...
        with PARK_LOCK:
            ...
            if park_state.get("high") is None or last_px > park_state.get("high", 0):
                park_state["high"] = last_px
                park_state["anchor"] = last_px
                park_state["last_change"] = time.time()
            base_high = park_state.get("high") if park_state.get("high") is not None else last_px
            # >>> usa trail_bps LOCAL (no el global)
            park_state["floor"] = base_high * (1.0 - (trail_bps * 0.0001))
            _TRADE_STATES[PARK_TRADE_ID] = park_state
            ...
            if qty_to_sell * btc_px >= MIN_NOTIONAL:
                ...
                try:
                    ...
                    with PARK_LOCK:
                        ...
                        park_state["reason"] = "floor_break"
                        park_state["parked"] = True
                        _TRADE_STATES[PARK_TRADE_ID] = park_state
                    return True
                except Exception as e:
    ...
    if PARK_PROTECT_ENABLED and btc_free <= 0.0 and usdt_free >= (MIN_NOTIONAL + 1.0):
        ...
        if btc_px >= reclaim_px:
            ...
            if buy_usdt >= MIN_NOTIONAL:
                # v28-fix: compute a basic score for park reclaim instead of bypassing with 0.0
                park_score = 0.0
                park_conf = 60
                try:
                    snap1h_pk = quick_tf_snapshot(symbol, "1h", limit=120) or {}
                    snap15_pk = quick_tf_snapshot(symbol, "15m", limit=120) or {}
                    pk_rsi1h = float(snap1h_pk.get("RSI") or 50.0)
                    pk_adx1h = float(snap1h_pk.get("ADX") or 0.0)
                    pk_rvol15 = float(snap15_pk.get("RVOL10") or snap15_pk.get("RVOL") or 0.0)
                    pk_macdh15 = float(snap15_pk.get("MACDh") or 0.0)
                    # Simple park score: require basic momentum confirmation
                    if pk_rsi1h >= 45: park_score += 1.5
                    if pk_adx1h >= 20: park_score += 1.5
                    if pk_rvol15 >= 0.8: park_score += 1.0
                    if pk_macdh15 > 0: park_score += 1.0
                    if pk_rsi1h >= 55: park_score += 0.5
                    if pk_adx1h >= 30: park_score += 0.5
                    park_conf = 65 if park_score >= 4.0 else 60
                except Exception:
                    park_score = 0.0
                # Block park reclaim if score too low
                if park_score < SCORE_GATE_HARD_MIN:
                    logger.info(f"[park-reclaim-blocked] {symbol}: park_score={park_score:.1f} < {SCORE_GATE_HARD_MIN} — skipping reclaim")
                    return False
                qty_to_buy = round(buy_usdt / btc_px, 8)
                try:
                    order = execute_order_buy(
                        symbol, qty_to_buy,
                        signals={"confidence": park_conf, "score": park_score},
                    )
                    if not order:
                        return False
                except Exception as e:
                ...
                with PARK_LOCK:
                    park_state = _TRADE_STATES.get(PARK_TRADE_ID, {})
                    park_state["last_change"] = now_epoch
                    park_state["reason"] = "reclaim"
                    park_state["parked"] = False
                    _TRADE_STATES[PARK_TRADE_ID] = park_state
                return True
...
def percent_spread(order_book: dict) -> Optional[float]:
    """Return spread as a fraction (e.g., 0.001 = 0.10%). Returns None if orderbook is missing/invalid."""
    try:
...
def longterm_regime(symbol: str) -> str:
    ...
    v31: Removed fear proxy override that forced 'bull' when RSI4h<35 — this was
    causing the bot to buy in bear markets. Now purely data-driven.
    """
...
def market_regime_ok() -> bool:
    """
    Check if market regime is OK for entries.
    v31: Removed fear proxy override — no more forcing regime_ok=True in fear.
    Now purely breadth-based: requires leader coins + minimum breadth count.
    """
    leaders_ok, breadth = compute_breadth()
    return bool(leaders_ok and breadth >= BREADTH_MIN_COUNT)


# >>> UNIFIED GET BTC RSI4H CACHED: Helper function with TTL 300s
def get_btc_rsi4h_cached():
    """
    Get cached RSI4h for BTC/USDT with TTL 300s.
    Returns None if fetch fails.
    """
    global _RSI4H_BTC_CACHE
    if time.time() - _RSI4H_BTC_CACHE.get('ts', 0) > 300:
        try:
            df4h = fetch_and_prepare_data_hybrid("BTC/USDT", timeframe="4h", limit=50)
            if df4h is not None and len(df4h) > 0:
                rsi_series = ta.rsi(df4h['close'], length=14)
                if len(rsi_series) > 0 and pd.notna(rsi_series.iloc[-1]):
                    _RSI4H_BTC_CACHE = {'rsi': float(rsi_series.iloc[-1]), 'ts': time.time()}
                else:
                    _RSI4H_BTC_CACHE = {'rsi': None, 'ts': time.time()}
            else:
                _RSI4H_BTC_CACHE = {'rsi': None, 'ts': time.time()}
        except Exception:
            _RSI4H_BTC_CACHE = {'rsi': None, 'ts': time.time()}
    return _RSI4H_BTC_CACHE.get('rsi')

# >>> HYBRID: Vol Slope Function (JSON edge: +1141 in success)
def calculate_vol_slope(df: pd.DataFrame, periods: int = 10) -> float:
    """
    Calculate volume slope from last periods (JSON edge: >0 for success).
    Returns linear slope of volume over last `periods` candles (positive = rising).
    """
    try:
        if df is None or len(df) < periods:
            return 0.0
        volumes = df['volume'].iloc[-periods:].values
        if len(volumes) < periods or np.any(np.isnan(volumes)):
            return 0.0
        x = np.arange(periods)
        slope = linregress(x, volumes).slope
        return float(slope) if not np.isnan(slope) else 0.0
    except Exception:
        return 0.0

def cooldown_ok(symbol: str, hours: int) -> bool:
    """
    Check if cooldown period has passed for symbol.
    Updates LAST_DCA timestamp if cooldown OK.
    """
    try:
        last_ts = LAST_DCA.get(symbol, 0)
        now = time.time()
        if now - last_ts > hours * 3600:
            LAST_DCA[symbol] = now
            return True
        return False
    except Exception:
...
# >>> BUILT-IN BACKTESTING (Simple, faster than Freqtrade CLI)
def calculate_max_dd(pnls: list) -> float:
    """
    Calculate maximum drawdown from a list of PnL values.
    Returns drawdown as a percentage.
    """
    try:
        if not pnls:
            return 0.0
...
def backtest_strategy() -> dict:
    """
    Simple built-in backtest (better than Freqtrade's heavy CLI).
    Simulates trades using historical data and returns metrics.
    """
    logger.info(f"Starting backtest from {BACKTEST_START} to {BACKTEST_END}")
    
    # Create a public exchange instance (no API keys needed for historical data)
    backtest_exchange = ccxt.binance({
        'enableRateLimit': True,
    ...
    try:
        ...
        start_dt = dt.strptime(BACKTEST_START, "%Y-%m-%d")
        end_dt = dt.strptime(BACKTEST_END, "%Y-%m-%d")
        since_ms = int(start_dt.timestamp() * 1000)
        until_ms = int(end_dt.timestamp() * 1000)
        
        # Initialize backtest state
        balance = 10000.0  # Starting balance in USDT
        initial_balance = balance
        trades = []
        positions = {}  # {symbol: {'entry_price': float, 'amount': float, 'entry_idx': int, 'mode': str}}
        pnls = []
        ...
        for symbol in SELECTED_CRYPTOS:
            ...
            try:
                ...
                while current < until_ms:
                    try:
                        ohlcv = backtest_exchange.fetch_ohlcv(symbol, BACKTEST_TIMEFRAME, since=current, limit=1000)
                        if not ohlcv:
                            break
                        ...
                        logger.warning(f"Error fetching {symbol} batch: {e}")
                        break
                ...
                if all_ohlcv:
                    ...
                    if len(df) >= 50:  # Need enough data for indicators (minimum ~50 candles)
                        # Calculate indicators (similar to fetch_and_prepare_data_hybrid)
                        for c in ['open', 'high', 'low', 'close', 'volume']:
                            df[c] = pd.to_numeric(df[c], errors='coerce').ffill().bfill()
                        ...
                        adx_df = ta.adx(df['high'], df['low'], df['close'], length=14)
                        df['ADX'] = adx_df['ADX_14'] if adx_df is not None and not adx_df.empty else np.nan
                        df['ATR'] = ta.atr(df['high'], df['low'], df['close'], length=14)
                        
                        rv_mean = df['volume'].rolling(10).mean()
                        df['RVOL10'] = (df['volume'] / rv_mean) if rv_mean.iloc[-1] > 0 else np.nan
                        
                        # Calculate slopes for last 10 periods
                        if len(df) >= 10:
                            for idx in range(10, len(df)):
                                x = np.arange(10)
                                prices_10 = df['close'].iloc[idx-10:idx].values
                                volumes_10 = df['volume'].iloc[idx-10:idx].values
                                if len(prices_10) == 10 and not np.any(np.isnan(prices_10)):
                                    df.loc[df.index[idx], 'PRICE_SLOPE10'] = linregress(x, prices_10).slope
                                else:
                                    df.loc[df.index[idx], 'PRICE_SLOPE10'] = np.nan
                                if len(volumes_10) == 10 and not np.any(np.isnan(volumes_10)):
                                    df.loc[df.index[idx], 'VOL_SLOPE10'] = linregress(x, volumes_10).slope
                                else:
                                    df.loc[df.index[idx], 'VOL_SLOPE10'] = np.nan
                        else:
        ...
        if not symbol_data:
            ...
            return {'trades': [], 'total_pnl': 0.0, 'win_rate': 0.0, 'max_dd': 0.0, 'total_return': 0.0}
        
        # Get all unique timestamps (align all symbols)
        all_timestamps = set()
        ...
        for i, ts in enumerate(all_timestamps):
            # Check exits first (trailing stop, time stop, etc.)
            symbols_to_close = []
            for symbol, pos in list(positions.items()):
                ...
                amount = pos['amount']
                gain_pct = ((current_price - entry_price) / entry_price * 100.0) if pos['mode'] != 'short' else ((entry_price - current_price) / entry_price * 100.0)
                
                # Simple exit logic (can be enhanced)
                exit_reason = None
                # Time stop (48h base)
                hours_held = (ts - pos['entry_ts']).total_seconds() / 3600.0
                if hours_held >= 48:
            ...
            # Check entries — v31: fixed max trades (no dynamic boost)
            if len(positions) >= MAX_OPEN_TRADES:
                continue
            
            for symbol in SELECTED_CRYPTOS:
                if symbol in positions or symbol not in symbol_data:
                    continue
                
                df = symbol_data[symbol]
                df_ts = df[df['timestamp'] <= ts]
                if len(df_ts) < 50:  # Need enough data
                    continue
                ...
                try:
                    ...
                    # v31: DCA only if daily trend is bullish
                    _bt_dca_lt = longterm_snapshot("BTC/USDT") if HYBRID_DCA_ENABLED else {}
                    _bt_dca_bull = (_bt_dca_lt.get("last") or 0) > (_bt_dca_lt.get("EMA200") or float('inf'))
                    if HYBRID_DCA_ENABLED and rsi4h < RSI_FEAR_PROXY and vol_slope > HYBRID_SHORT_VOL_SLOPE_MIN and _bt_dca_bull:
                        mode = 'dca'
                    ...
                    if mode:
                        ...
                        try:
                            # Get RSI4h for BTC and FGI - use cached helper
        ...
        logger.error(f"backtest_strategy error: {e}", exc_info=True)
        return {'trades': [], 'total_pnl': 0.0, 'win_rate': 0.0, 'max_dd': 0.0, 'total_return': 0.0}

# >>> PARAM OPTIMIZATION (Grid search, simpler than Freqtrade Optuna)
def optimize_params() -> dict:
    """
    Optimize parameters using grid search on backtest results.
    """
    ...
    # Save original values and declare globals
    global RSI_MIN_DEFAULT, ADX_MIN_DEFAULT, HYBRID_SHORT_VOL_SLOPE_MIN
    original_rsi_min = RSI_MIN_DEFAULT
    original_adx_min = ADX_MIN_DEFAULT
    original_vol_slope_min = HYBRID_SHORT_VOL_SLOPE_MIN
    
    try:
        for rsi in OPT_PARAMS['rsi_low']:
            for adx in OPT_PARAMS['adx_min']:
                for vol in OPT_PARAMS['vol_slope_min']:
                    # Set globals temporarily
                    RSI_MIN_DEFAULT = rsi
                    ADX_MIN_DEFAULT = adx
                    HYBRID_SHORT_VOL_SLOPE_MIN = vol
                    
                    logger.info(f"Testing params: RSI={rsi}, ADX={adx}, VOL_SLOPE={vol}")
                    results = backtest_strategy()
                    
                    if results and results['total_pnl'] > best_pnl:
        ...
        RSI_MIN_DEFAULT = original_rsi_min
        ...
        logger.error(f"optimize_params error: {e}")
        # Restore original values
        RSI_MIN_DEFAULT = original_rsi_min
        ADX_MIN_DEFAULT = original_adx_min
        HYBRID_SHORT_VOL_SLOPE_MIN = original_vol_slope_min
...
# >>> ML LIGHT (Simple scikit-learn, no heavy deps like FreqAI)
def train_ml_model(historical_trades: list) -> object | None:
    """
    Train ML model on historical trades (RandomForest).
    """
    if not SKLEARN_AVAILABLE or not historical_trades:
        return None
    
    try:
        # Extract features from trades (need to store features in trades)
        X = []
        y = []
        
        for trade in historical_trades:
            # Get features from trade (if stored)
            rsi = trade.get('rsi', 50.0)
            adx = trade.get('adx', 20.0)
            vol_slope = trade.get('vol_slope', 0.0)
            
            X.append([rsi, adx, vol_slope])
            y.append(1 if trade.get('pnl', 0) > 0 else 0)  # Win/loss
        ...
        logger.info(f"ML model trained, accuracy: {score*100:.2f}%")
        
        return model
    except Exception as e:
        logger.error(f"train_ml_model error: {e}")
        return None

def ml_predict(model: object, features: dict) -> float:
    """
    Predict win probability using ML model.
    Returns probability (0-1) of winning trade.
    """
    ...
    try:
        ...
        vol_slope = features.get('vol_slope', 0.0)
        
        pred_proba = model.predict_proba([[rsi, adx, vol_slope]])[0][1]  # Probability of win
        return float(pred_proba)
    except Exception as e:
        logger.debug(f"ml_predict error: {e}")
        return 0.5

# >>> HYBRID PREFLIGHT: Determines entry mode (DCA, short, or swing)
def hybrid_preflight(symbol: str) -> tuple[str | None, str]:
    """
    ...
    """
    try:
        df1h = fetch_and_prepare_data_hybrid(symbol, timeframe="1h", limit=50)
        df4h = fetch_and_prepare_data_hybrid(symbol, timeframe="4h", limit=50)
        
        if df1h is None or len(df1h) < 14 or df4h is None or len(df4h) < 14:
            return None, "insufficient data"
        
        try:
            rsi4h = float(df4h['RSI'].iloc[-1]) if pd.notna(df4h['RSI'].iloc[-1]) else 50.0
            rsi1h = float(df1h['RSI'].iloc[-1]) if pd.notna(df1h['RSI'].iloc[-1]) else 50.0
        ...
        # >>> DCA Mode: Fear proxy (RSI4h BTC < 35) + vol rising + RVOL15 check
        # v31: DCA only if daily trend is bullish — no DCA in bear markets
        if HYBRID_DCA_ENABLED and rsi4h_btc is not None and rsi4h_btc < 35 and vol_slope > HYBRID_SHORT_VOL_SLOPE_MIN:
            # v31: Check daily trend before DCA
            try:
                _dca_lt = longterm_snapshot("BTC/USDT")
                _dca_px = _dca_lt.get("last") if _dca_lt else None
                _dca_ema = _dca_lt.get("EMA200") if _dca_lt else None
                if _dca_px is not None and _dca_ema is not None and _dca_px < _dca_ema:
                    return None, f"DCA blocked: bear market (BTC {_dca_px:.0f} < EMA200 {_dca_ema:.0f})"
            except Exception:
                pass
...
def preflight_buy_guard(symbol: str) -> Tuple[bool, str]:
    ...
    try:
        if len(df1h) >= 10:
            volumes_10 = df1h['volume'].iloc[-10:].values
            if len(volumes_10) == 10 and not np.any(np.isnan(volumes_10)):
                x = np.arange(10)
                vol_slope = linregress(x, volumes_10).slope
                if vol_slope <= 0:
                    return False, f"Vol slope not rising ({vol_slope:.2f}) - skip low quality (JSON edge requires >0)"
    except Exception as e:
        logger.debug(f"vol_slope check error {symbol}: {e}")
    ...
    # (A0) Anti-FOMO guard: avoid buying late spikes without healthy participation.
    if rsi15 is not None and rsi15 >= 70.0:
        if (rvol1h is None or rvol1h < 1.0) and (macdh15_now is None or macdh15_now <= 0.0):
            return False, f"late momentum / weak participation (RSI15={rsi15:.1f}, RVOL1h={rvol1h}, MACDh15={macdh15_now})"
...
def controller_autotune():
    try:
        ...
        try:
            # Get RSI4h for BTC and vol_slope - use cached helper
            rsi4h_btc = get_btc_rsi4h_cached()
...
def hybrid_decision(symbol: str):
    ...
    try:
        ...
        # >>> FGI OVERRIDE: v30 — NO longer relax thresholds in extreme fear
        # v29 lowered RSI_MIN to 30 and ADX_MIN to 20 in extreme fear, causing falling knife entries.
        # v30: In extreme fear (FGI <= 20), KEEP strict thresholds. Only relax in mild fear (20 < FGI <= 35).
        if fgi_classification in ["fear", "extreme_fear"] and fgi_slope_7d is not None and fgi_slope_7d > 0:
            if fgi_value is not None and fgi_value <= 20:  # Extreme fear
                # v30: Do NOT relax — keep standard thresholds, just log
                blocks.append(f"FGI extreme fear (FGI={fgi_value:.0f}, slope_7d={fgi_slope_7d:+.1f}) - v30: NO threshold relaxation")
            else:
                blocks.append(f"FGI override: Fear recovering (FGI={fgi_value:.0f}, slope_7d={fgi_slope_7d:+.1f}) - allowing entries")
    ...
    try:
        ...
        if spr_p is None:
            blocks.append("orderbook unavailable")
            level = "HARD"
        elif spr_p > SPREAD_MAX_PCT_DEFAULT:
    ...
    # RSI: avoid starving trend swings. Only hard-block extremes; allow RSI>55 if trend-confirmed.
    try:
        ema20_now = float(row['EMA20']) if pd.notna(row.get('EMA20')) else None
        ema50_now = float(row['EMA50']) if pd.notna(row.get('EMA50')) else None
        close_now = float(row['close']) if pd.notna(row.get('close')) else None
        in_lane_now = bool(ema20_now and ema50_now and close_now and (ema20_now > ema50_now) and (close_now > ema20_now))
    except Exception:
        in_lane_now = False
    trend_confirmed = bool(in_lane_now or (breakout and retest_ok) or ((adx or 0) >= 25.0))

    if rsi is None:
        blocks.append("HARD block: RSI is None")
        level = "HARD"
    elif rsi < 20.0:
        blocks.append(f"HARD block: RSI too low ({rsi:.1f}, need ≥20)")
        level = "HARD"
    elif rsi > 75.0:
        blocks.append(f"HARD block: RSI too high ({rsi:.1f}, need ≤75)")
        level = "HARD"
    elif rsi > 55.0 and not trend_confirmed:
        blocks.append(f"SOFT block: RSI high without trend confirmation ({rsi:.1f} > 55)")
        if level != "HARD": level = "SOFT"
    elif rsi < 25.0:
        blocks.append(f"SOFT block: RSI low ({rsi:.1f} < 25) — caution")
        if level != "HARD": level = "SOFT"
    ...
    if volume_slope10 is None or volume_slope10 <= 0:
        ...
        try:
            rsi4h_btc_vol = get_btc_rsi4h_cached()  # Use cached helper
    ...
    # For swing/trend entries, treat low RVOL15 as SOFT when 1h lane is bullish or breakout+retest is present.
    if not (adx4h_now and adx4h_now >= 25 and adx4h_rising_gate):
        if (rv15_closed or 0) < RVOL15_ENTRY_MIN:
            try:
                in_lane_early = bool(row['EMA20'] > row['EMA50'] and row['close'] > row['EMA20'])
            except Exception:
                in_lane_early = False
            if in_lane_early or (breakout and retest_ok) or ((rvol_1h or 0) >= 1.3):
                blocks.append(f"SOFT block: RVOL15_closed {rv15_closed} < {RVOL15_ENTRY_MIN} (thin tape)")
                if level != "HARD": level = "SOFT"
            else:
    ...
    if proxy_active_entry and (rsi4h_slope < -0.5):
        # Strong bearish: block only if slope is very negative (falling fast)
        blocks.append(f"SOFT block: RSI4h={rsi4h:.1f}<{RSI_FEAR_PROXY} with strong negative slope ({rsi4h_slope:.2f})")
        if level != "HARD": level = "SOFT"
    ...
    if macd15_leq0 and weak15:
        # Swing-friendly: don't hard-veto on 15m MACD if higher-TF trend/support exists
        try:
            in_lane_15m_ok = bool(row['EMA20'] > row['EMA50'] and row['close'] > row['EMA20'])
        except Exception:
            in_lane_15m_ok = False
        higher_tf_support = bool(in_lane_15m_ok or (breakout and retest_ok) or ((rvol_1h or 0) >= 1.3) or ((rv15 or 0) >= 1.2))
        if higher_tf_support:
            blocks.append("SOFT block: weak 15m momentum (MACDh15≤0 & (RSI15≤50 or ADX1h<25)) — allowed by higher-TF support")
            if level != "HARD": level = "SOFT"
        else:
    ...
    # v30-fix: Mean-reversion bonus DISABLED in extreme fear (FGI <= 20)
    # In extreme fear, oversold RSI = falling knife, not reversal opportunity
    _mr_fgi = None
    try:
        _mr_fgi_f = fetch_fgi_features()
        _mr_fgi = _mr_fgi_f.get("value") if _mr_fgi_f else None
    except Exception:
        pass
    if _mr_fgi is None or _mr_fgi > FGI_EXTREME_FEAR:
        ...
        if rsi < 25.0:
            notes.append(f"RSI oversold BLOCKED (FGI={_mr_fgi}<=20, falling knife)")
    ...
    # v30-fix: Fetch FGI for conviction gating — no mean reversion in extreme fear
    _conv_fgi = None
    try:
        _conv_fgi_feats = fetch_fgi_features()
        _conv_fgi = _conv_fgi_feats.get("value") if _conv_fgi_feats else None
    except Exception:
        pass
    ...
    if level != "HARD":
        ...
        # v30-fix: BLOCKED when FGI <= 20 (extreme fear) — catching falling knives loses money
        rsi4h_conv = None
        try:
            rsi4h_conv = get_btc_rsi4h_cached()
        except Exception:
            pass
        if (_conv_fgi is not None and _conv_fgi > FGI_EXTREME_FEAR and
                rsi4h_conv is not None and rsi4h_conv < CONVICTION_RSI4H_DEEP_OVERSOLD):
        ...
        elif rsi4h_conv is not None and rsi4h_conv < CONVICTION_RSI4H_DEEP_OVERSOLD and _conv_fgi is not None and _conv_fgi <= FGI_EXTREME_FEAR:
            notes.append(f"CONV-BLOCKED:mean_rev_in_extreme_fear(FGI={_conv_fgi},RSI4h={rsi4h_conv:.1f})")
    ...
    # v30-fix: score_for_gate set AFTER conviction override (was before, causing Score=0.0 reports)
...
def execute_order_buy(symbol: str, amount: float, signals: dict):
    try:
        try:
            ...
            logger.error(f"Exchange order error {symbol}: {e}")
            return None
        ...
        try:
            ...
            logger.error(f"Price/filled extraction error {symbol}: {e}")
            return None
        ...
        try:
            ...
            logger.warning(f"Features save error {symbol}: {e}")
...
def execute_order_sell(symbol: str, amount: float, signals: dict | None = None) -> dict | None:
    ...
    """
    try:
        # 1) Precio de referencia y orden (cap by available base balance)
        price_now = fetch_price(symbol)
        base = symbol.split('/')[0]
        try:
            bal = exchange.fetch_balance() or {}
            free_base = float((bal.get("free", {}) or {}).get(base, 0.0) or 0.0)
            amount = min(float(amount), free_base * 0.998)  # tiny cushion for precision
        except Exception:
            amount = float(amount)

        if amount <= 0:
            return None
        if price_now and (amount * float(price_now)) < MIN_NOTIONAL:
            return None

        order = exchange.create_market_sell_order(symbol, amount)
        # v30-fix: Use actual filled amount, not requested amount
...
def execute_short(symbol: str, amount: float, signals: dict):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Price/filled extraction error (short) {symbol}: {e}")
            return None
        
        trade_id = f"{symbol}-SHORT-{datetime.now(timezone.utc).isoformat().replace(':','-')}"
        ...
        try:
            ...
            logger.error(f"DB insert error (short) {symbol}: {e}")
            return None
        
        try:
            send_telegram_message(f"📉 SHORT {symbol}\nPrice: {price}\nAmount: {filled}\nConf: {signals.get('confidence', 0)}%\nScore: {signals.get('score', 0):.1f}")
        except Exception as e:
            logger.warning(f"Telegram send error (short) {symbol}: {e}")
        ...
        try:
            ...
            logger.warning(f"Features save error (short) {symbol}: {e}")
        
        return {"price": price, "filled": filled, "trade_id": trade_id}
    except Exception as e:
        logger.error(f"execute_short error {symbol}: {e}")
        try:
            send_telegram_message(f"❌ SHORT failed {symbol}: {e}")
        except Exception:
            pass
        return None

def close_short(symbol: str, amount: float, trade_id: str, source: str = "trail"):
    """
    Close short position (buy to close).
    Similar to sell_symbol but buys to close shorts.
    """
    try:
        price_now = fetch_price(symbol)
        order = exchange.create_market_buy_order(symbol, amount)  # Buy to close short
        close_price = order.get("price", price_now) or price_now or 0.0
        close_ts = datetime.now(timezone.utc).isoformat()
        
        conn = sqlite3.connect(DB_NAME); cur = conn.cursor()
        cur.execute("""
            INSERT INTO transactions (symbol, side, price, amount, ts, trade_id, status)
            VALUES (?, 'close_short', ?, ?, ?, ?, 'closed')
        """, (symbol, float(close_price), float(amount), close_ts, trade_id))
        cur.execute("UPDATE transactions SET status='closed' WHERE trade_id=? AND side='short'", (trade_id,))
        conn.commit(); conn.close()
        
        send_telegram_message(f"✅ CLOSE SHORT {symbol}\nPrice: {close_price}\nAmount: {amount}\nSource: {source}")
        LAST_SELL_INFO[symbol] = {"ts": close_ts, "price": float(close_price), "source": source}
        LAST_TRADE_CLOSE[symbol] = close_ts
        
        features_exit = build_rich_features(symbol)
        save_trade_features(trade_id, symbol, 'exit', features_exit)
        
        logger.info(f"Closed short {symbol}: amt={amount}, px={close_price}, trade_id={trade_id}, source={source}")
    except Exception as e:
        logger.error(f"close_short error {symbol}: {e}")
        try:
            send_telegram_message(f"❌ CLOSE SHORT failed {symbol}: {e}")
        except Exception:
            pass
...
def dynamic_trailing_stop(symbol: str, amount: float, purchase_price: float, trade_id: str, atr_abs: float, mode: str = 'momentum'):
    ...
    def loop():
        ...
        try:
            ...
            # Adjust k for DCA mode (widen trailing: k*=1.5)
            if mode == 'dca':
                base_k = base_k * 1.5
                logger.debug(f"[trailing-dca] {symbol}: Widened k to {base_k:.2f} (DCA mode)")
            ...
            CATA_MAX_DD_BPS_ALT = int(os.getenv("CATA_MAX_DD_BPS_ALT", "500"))  # v28-fix: 5.0% ALTs (was 6.0% — ADA lost 6.64% before hitting floor)
            cata_dd_bps = CATA_MAX_DD_BPS_BTC if base in {"BTC", "ETH"} else CATA_MAX_DD_BPS_ALT
            # v28-fix: tighten catastrophic floor for long-held trades (inherited/legacy)
            # Trades held >48h should have tighter stops since they've had time to trend
            try:
                _trade_age_h = (datetime.now(timezone.utc) - opened_ts).total_seconds() / 3600.0
                if _trade_age_h > 48:
                    # Tighten by 50bps for every 24h beyond 48h, min floor 250bps
                    _extra_tighten = int(min((_trade_age_h - 48) / 24.0, 4) * 50)
                    cata_dd_bps = max(250, cata_dd_bps - _extra_tighten)
                    logger.debug(f"[cata-tighten] {symbol}: trade age {_trade_age_h:.0f}h, cata tightened by {_extra_tighten}bps to {cata_dd_bps}bps")
            except Exception:
                pass
            ...
            while True:
              try:  # v28: per-iteration try/except — transient errors continue, thread never dies
                ...
                # Keep local amount synced with current net position after partials.
                ...
                # Track highest price for new high check (unified per trade_id)
                if not hasattr(loop, '_trade_states'):
                    loop._trade_states = {}
                state = loop._trade_states.get(trade_id, {})
                pmeta = state.setdefault('partial_meta', {"count": 0, "last_ts": 0.0, "reasons": {}})
                state['highest_price'] = max(state.get('highest_price', purchase_price), last_price)
                ...
                # >>> UNIFIED GET BTC RSI4H CACHED: Use helper function
                rsi4h_btc = get_btc_rsi4h_cached()
                ...
                if gain_pct > rapid_kill_threshold:
                    ...
                    if mode == 'dca' or (rsi4h_btc is not None and rsi4h_btc < 35):
                        ...
                        # Only kill if vol_slope is not None and < 0 (fail-open: skip if None)
                            ...
                            fees_bps = FEE_BPS_PER_SIDE * 2  # buy + sell
                            be_floor = entry_price * (1.0 + (fees_bps / 10000.0))  # BE + fees
                            set_symbol_trailing(symbol, absolute_floor=be_floor, ttl_sec=3600)  # 1 hour TTL
                            logger.info(f"Tighten to BE+fees: {symbol} +{gain_pct:.2f}% estancamiento (BE+fees={be_floor:.4f}, fees={fees_bps}bps)")
                            
                            # >>> DETERIORATION CHECK: Only sell if macdh_15m < 0 and rsi_slope_15m < 0 and drawdown_bps > 50
                            try:
                                df15m_stall = fetch_and_prepare_data_hybrid(symbol, timeframe="15m", limit=20)
                                macdh_15m = None
                                rsi_slope_15m = None
                                if df15m_stall is not None and len(df15m_stall) >= 14:
                                    macd15_df = ta.macd(df15m_stall['close'], fast=12, slow=26, signal=9)
                                    if macd15_df is not None and not macd15_df.empty:
                                        macdh_15m = float(macd15_df['MACDh_12_26_9'].iloc[-1]) if pd.notna(macd15_df['MACDh_12_26_9'].iloc[-1]) else None
                                    rsi_slope_15m = rsi_slope(df15m_stall, length=14, bars=6)
                                
                                drawdown_bps = max(0.0, (entry_price - last_price) / entry_price * 10000) if entry_price > 0 else 0.0
                                
                                if macdh_15m is not None and rsi_slope_15m is not None:
                                    if macdh_15m < 0 and rsi_slope_15m < 0 and drawdown_bps > 50:
                                        # Deterioration detected: sell
                                        logger.info(f"Deterioration sell: {symbol} macdh_15m={macdh_15m:.4f}<0, rsi_slope_15m={rsi_slope_15m:.2f}<0, drawdown={drawdown_bps:.0f}bps>50")
                                        if mode == 'short':
                                            close_short(symbol, amount_to_sell, trade_id=trade_id, source="stagnation-deterioration")
                                        else:
                                            sell_symbol(symbol, amount_to_sell, trade_id=trade_id, source="stagnation-deterioration")
                                        cleanup_trade_state()
                                        time.sleep(EXIT_CHECK_EVERY_SEC)
                                        continue
                            except Exception as e:
                                logger.debug(f"Stagnation deterioration check error: {e}")
                            
                            # No sell, let trail continue
                            pass

                # v28-fix: Corte duro — was 40min/-0.9%, now 90min/-2.5% (trades need time to develop)
                if minutes_since_entry > 90 and gain_pct < -2.5:
                ...
                try:
                    if PREARM_ENABLE and not trail_armed:
                        ...
                        # Arm trailing first after timeout; avoid late prearm-stop exits.
                        if minutes_open >= ARM_TIME_MAX_MIN:
                            trail_armed = True
                            if mode != 'short' and prearm_floor is not None:
                                set_symbol_trailing(symbol, trail_bps=None, absolute_floor=prearm_floor)
                            logger.info(f"[prearm-arm] {symbol} auto-armed after {minutes_open:.1f}m")
                            # From now on, trailing logic handles exits.
                            continue
                        
                        # >>> RAPID/PREARM GRACE IN FEAR: If RSI4h<35 and gain>0.2%, delay 30min
                        rsi4h_btc_prearm = None
                        # Use cached helper
                        rsi4h_btc_prearm = get_btc_rsi4h_cached()
                ...
                if (not RAPID_KILL_ENABLED) or mode == 'dca':
                    ...
                    try:
                        ...
                        # Use cached helper
                        rsi4h_btc_rapid = get_btc_rsi4h_cached()
                ...
                # >>> SHORT MODE: Inverse stop check (for shorts, exit if price > stop)
                if mode == 'short':
                    # For shorts: exit if price exceeds stop (price goes up = loss)
                    if price >= initial_stop:
                        close_short(symbol, amount, trade_id, source="trail-stop")
                        return
                    # Track lowest price for inverse trailing (unified per trade_id)
                    if not hasattr(loop, '_trade_states'):
                        loop._trade_states = {}
                    state = loop._trade_states.get(trade_id, {})
                    if 'lowest_price' not in state:
                        state['lowest_price'] = purchase_price
                    state['lowest_price'] = min(state.get('lowest_price', purchase_price), price)
                    loop._trade_states[trade_id] = state
                
                # Global min-hold except hard protective stop (for longs)
                if not held_long_enough:
                    if mode != 'short' and price <= initial_stop:
                ...
                # >>> ATR% REAL AND TP THRESHOLD: Calculate ATR% real and TP threshold based on R_pct
                ...
                if hold_info["hold"]:
                    ...
                    if hold_info["partial"] > 0 and gain_pct_now >= 1.0:  # At least 1R
                        try:
                            now_ts = time.time()
                            rs = (pmeta.get("reasons", {}) or {})
                            too_many = int(pmeta.get("count", 0)) >= MAX_PARTIAL_SELLS_PER_TRADE
                            in_cooldown = (now_ts - float(pmeta.get("last_ts", 0.0))) < PARTIAL_SELL_COOLDOWN_SEC
                            already_done = int(rs.get("volume-hold-partial", 0)) >= 1
                            if not too_many and not in_cooldown and not already_done:
                                ...
                                if partial_amt * price >= MIN_NOTIONAL and partial_amt > 0:
                                    sold = execute_order_sell(symbol, partial_amt, signals={"reason": "volume-hold-partial"})
                                    if sold:
                                        pmeta["count"] = int(pmeta.get("count", 0)) + 1
                                        pmeta["last_ts"] = now_ts
                                        rs["volume-hold-partial"] = int(rs.get("volume-hold-partial", 0)) + 1
                                        pmeta["reasons"] = rs
                                        state["partial_meta"] = pmeta
                                        loop._trade_states[trade_id] = state
                ...
                if gain_pct_now >= 4.0:
                    try:
                        ...
                        if not hasattr(loop, '_partial_taken'):
                            loop._partial_taken = False
                        
                        if not loop._partial_taken:
                            pos_qty = current_position_qty(symbol)
                            qty_partial = round(pos_qty * 0.30, 8)  # 30% partial
                            if qty_partial * price >= MIN_NOTIONAL and qty_partial > 0:
                                sold = execute_order_sell(symbol, qty_partial, signals={"reason": "partial-profit-4pct"})
                                if sold:
                                    loop._partial_taken = True
                                    rs = (pmeta.get("reasons", {}) or {})
                                    pmeta["count"] = int(pmeta.get("count", 0)) + 1
                                    pmeta["last_ts"] = time.time()
                                    rs["partial-profit-4pct"] = int(rs.get("partial-profit-4pct", 0)) + 1
                                    pmeta["reasons"] = rs
                                    state["partial_meta"] = pmeta
                                    loop._trade_states[trade_id] = state
                ...
                # v28-fix: ADAPTIVE trailing ATR multiple by volatility regime
                # Research (arXiv 2602.11708): optimal alpha=2.5 (range 2.0-3.5x)
                # Compare current ATR to its SMA(50) to detect volatility regime
                _vol_regime = "normal"
                try:
                    _atr_series = df1h['ATR'].dropna()
                    if len(_atr_series) >= 50:
                        _atr_sma50 = _atr_series.rolling(50).mean().iloc[-1]
                        _atr_current = _atr_series.iloc[-1]
                        if _atr_sma50 > 0:
                            _atr_ratio = _atr_current / _atr_sma50
                            if _atr_ratio < 0.8:
                                _vol_regime = "low"
                                base_mult = 2.5   # v31: widened from 2.0 for better R:R
                            elif _atr_ratio < 1.2:
                                _vol_regime = "normal"
                                base_mult = 3.0   # v31: widened from 2.5
                            elif _atr_ratio < 1.5:
                                _vol_regime = "high"
                                base_mult = 3.5   # v31: widened from 3.0
                            else:
                                _vol_regime = "extreme"
                                base_mult = 4.0   # v31: widened from 3.5
                        else:
                            base_mult = 3.0  # v31: widened from 2.5
                    else:
                        base_mult = 3.0  # v31: widened from 2.5
                ...
                # >>> PROXY MODULATION: If RSI4h < RSI_FEAR_PROXY, widen trailing (k*=1.3)
                proxy_active_trail = False
                try:
                    rsi4h_proxy_trail = get_btc_rsi4h_cached()  # Use cached helper
                    if rsi4h_proxy_trail is not None and rsi4h_proxy_trail < RSI_FEAR_PROXY:
                            proxy_active_trail = True
                            base_mult = base_mult * 1.3  # v28-fix: was 1.5, now 1.3 (less extreme widening)
                            logger.debug(f"Proxy active in trailing: RSI4h={rsi4h_proxy_trail:.1f}<{RSI_FEAR_PROXY}, base_mult widened to {base_mult:.2f}")
                except Exception as e:
                    logger.debug(f"Proxy check in trailing error: {e}")

                # If gain >= 10%: tighten trailing (capture more profit on strong moves)
                if gain_pct_now >= 10.0:
                    base_mult = 1.5  # v28-fix: was 1.2 — still tighten but not too aggressively

                # >>> FGI modulation (only in extremes with slope)
                if fgi_classification == "extreme_greed" and fgi_slope_7d is not None and fgi_slope_7d > 0:
                    trail_atr_mult = min(base_mult, 1.8)  # v28-fix: was 1.2, now cap at 1.8 (euphoria tighter but not too much)
                elif fgi_classification in ["fear", "extreme_fear"] and fgi_slope_7d is not None and fgi_slope_7d > 0:
                    trail_atr_mult = max(base_mult, 2.5)  # v28-fix: was 1.8, now at least 2.5 (fear recovering: let breathe)
                else:
                    trail_atr_mult = base_mult

                logger.debug(f"[adaptive-trail] {symbol}: vol_regime={_vol_regime} base_mult={base_mult:.2f} trail_atr_mult={trail_atr_mult:.2f} proxy={proxy_active_trail}")
                ...
                # Track highest price for MFE calculation (for kill switch) - unified per trade_id
                if not hasattr(loop, '_trade_states'):
                    loop._trade_states = {}
                state = loop._trade_states.get(trade_id, {})
                state['highest_price'] = max(state.get('highest_price', purchase_price), price)
                loop._trade_states[trade_id] = state
                ...
                try:
                    regime_trending_trail = bool(strong_trend or market_regime_ok())
                except Exception:
                    regime_trending_trail = False
                ...
                # >>> EMERGENCY_CUT TIGHTER (Trailing): v28-fix: -1.5%/60min → -2.5%/90min (was cutting too early)
                if mode != 'short':  # Only for longs
                    if minutes_since_entry_trail >= 90 and gain_pct_now < -2.5:
                ...
                if mode == 'short':
                    ...
                    # Inverse trailing: stop = lowest_price + trail_pct (above entry for shorts)
                    trail_pct_short = trail_atr_mult * atr_pct  # Same ATR multiple
                    candidate_stop_short = state['lowest_price'] * (1.0 + trail_pct_short / 100.0)  # Above lowest
                    
                    # Take minimum stop (most protective for shorts = lowest stop above entry)
                    stop_candidates_short = [initial_stop, candidate_stop_short]
                    if be_stop_candidate is not None:
                        # For shorts: BE is entry price (no profit yet)
                        be_stop_candidate_short = purchase_price  # Break-even for shorts
                        stop_candidates_short.append(be_stop_candidate_short)
                    
                    # Take minimum (most protective = closest to entry for shorts)
                    stop_price = min(stop_candidates_short)
                    
                    # Ensure stop never goes below entry price (for shorts, stop must be above)
                    stop_price = max(stop_price, purchase_price)
                    
                    # Exit if price exceeds stop (for shorts, price going up = loss)
                    if price >= stop_price:
                        close_short(symbol, amount, trade_id, source="trail-stop-short")
                        return
                else:
                    # Long mode: normal trailing
                    # Ensure stop never goes below initial stop
                    stop_price = max(stop_price, initial_stop)
                    
                    # Exit if price hits stop (for longs, price going down = loss)
                    if price <= stop_price:
                        sell_symbol(symbol, amount, trade_id, source="trail-stop")
                        return

                # >>> RVOL COLLAPSE EXIT: Disabled in fear (vol dries up in bottoms, but rebounds come)
                # Skip RVOL collapse exit for shorts (different logic needed)
                if mode == 'short':
                    pass  # Skip RVOL collapse for shorts
                elif RVOL_COLLAPSE_EXIT_ENABLED:
                    # In fear (FGI < 20), set threshold very low (0.3) to avoid premature exits
                    rvol_collapse_threshold = RVOL_COLLAPSE_EXIT
                try:
                    fgi_features = fetch_fgi_features()
                    fgi_value = fgi_features.get("value")
                    if fgi_value is not None and fgi_value <= 20:
                        rvol_collapse_threshold = 0.3  # Much lower threshold in fear
                        logger.debug(f"[rvol-collapse] {symbol}: Threshold lowered to {rvol_collapse_threshold} (FGI={fgi_value})")
                except Exception:
                    pass
                
                # Volume-collapse exit (15m closed) — skip in strong trend
                # >>> STRONG_TREND_ADX_1H: Skip RVOL collapse exit in strong trends (let winners run)
                # >>> STRONG_TREND_ADX_1H: Skip RVOL collapse exit in strong trends (let winners run)
                if RVOL_COLLAPSE_EXIT_ENABLED and not strong_trend:
                    try:
                        ...
                        if held_long_enough and (rvol15_closed is not None) and (rvol15_closed < rvol_collapse_threshold):
                            logger.info(f"[collapse-exit] {symbol} 15m RVOL(closed)={rvol15_closed:.2f} < {rvol_collapse_threshold:.2f} — exiting.")
                            ...
                            try:
                                ...
                                if rvol_ok and structure_up:
                                    ...
                                    too_many = int(pmeta.get("count", 0)) >= MAX_PARTIAL_SELLS_PER_TRADE
                                    in_cooldown = (now_ts - float(pmeta.get("last_ts", 0.0))) < PARTIAL_SELL_COOLDOWN_SEC
                                    if (not too_many) and (not in_cooldown) and qty_partial * last_price >= MIN_NOTIONAL and qty_partial > 0:
                                        sold = execute_order_sell(symbol, qty_partial, signals={"reason": "volume-partial"})
                                        if sold:
                                            pmeta["count"] = int(pmeta.get("count", 0)) + 1
                                            pmeta["last_ts"] = now_ts
                                            rs["volume-partial"] = int(rs.get("volume-partial", 0)) + 1
                                            pmeta["reasons"] = rs
                                            state["partial_meta"] = pmeta
                                            loop._trade_states[trade_id] = state
                ...
                # >>> TIME-STOP: Extended for full swing (capture rebounds)
                # Base: 48h (2 days), Extended: 120h (5 days) if vol_slope>0 or RSI<40
                # JSON success in dips: RSI<40 and vol_slope>0 are key conditions
                time_stop_hours = TIME_STOP_HOURS  # Base 48h (2 days)
                try:
                    # Check volume slope (from JSON: vol_slope positive in successful trends)
                    vol_slope_positive = False
                    rsi_low = False
                    try:
                        df15_vol = fetch_and_prepare_data_hybrid(symbol, timeframe="15m", limit=20)
                        if df15_vol is not None and len(df15_vol) >= 10:
                            volumes_10 = df15_vol['volume'].iloc[-10:].values
                            x = np.arange(10)
                            vol_slope = linregress(x, volumes_10).slope
                            vol_slope_positive = vol_slope > 0
                        
                        # Check RSI (JSON success in dips: RSI<40)
                        rsi1h_val = float(df1h['RSI'].iloc[-1]) if pd.notna(df1h['RSI'].iloc[-1]) else None
                        if rsi1h_val is not None and rsi1h_val < 40:
                            rsi_low = True
                    except Exception:
                        pass
                    
                    # Extend to 120h (5 days) if vol_slope>0 or RSI<40 (JSON edge conditions)
                    if vol_slope_positive or rsi_low:
                        time_stop_hours = TIME_STOP_EXTEND_HOURS  # 120h (5 days)
                        logger.debug(f"[time-stop] {symbol}: Extended to {time_stop_hours}h (vol_slope={vol_slope_positive}, RSI<40={rsi_low})")
                except Exception:
                    pass
                
                # Time-stop if MACDh15 < 0 on two closed bars (after extended period)
                ...
                # >>> PRICE-TRIGGERED EXIT: Different logic for shorts vs longs
                # For shorts: exit if price >= stop_price (price goes up = loss)
                # For longs: exit if price <= stop_price (price goes down = loss)
                if mode == 'short':
                    if price >= stop_price:
                        close_short(symbol, amount, trade_id, source="trail-stop")
                        return
                        ...
                        if gain_pct < edge_needed_pct:
                            ...
                            if gain_pct < edge_needed_pct:
                                ...
                                if structural_confirmed:
                                    if mode == 'short':
                                        close_short(symbol, amount, trade_id, source="trail-structure")
                                    else:
                                        sell_symbol(symbol, amount, trade_id, source="trail")
                                    return
                            ...
                            if mode == 'short':
                                close_short(symbol, amount, trade_id, source="trail")
                            else:
                                sell_symbol(symbol, amount, trade_id, source="trail")
                            return
                    else:
                ...
                # Disable if RSI<45 (swing volatility - don't wait for rebounds in dips)
                ...
                try:
                    rsi4h_btc_rebound = get_btc_rsi4h_cached()  # Use cached helper
...
def size_position(price: float, usdt_balance: float, confidence: int, symbol: str = None) -> float:
    ...
    # v30-fix: Aggressive position reduction in extreme fear.
    # v29 used 0.65x for FGI<=10 and BOOST for FGI>10 — too generous in panic markets.
    try:
        rsi4h_btc = get_btc_rsi4h_cached()
        fgi_features = fetch_fgi_features()
        fgi_v = fgi_features.get("value") if fgi_features else None
        if fgi_v is not None and fgi_v <= 10:
            base_frac *= 0.30  # v30: 30% size in extreme panic (was 0.65)
        elif fgi_v is not None and fgi_v <= FGI_EXTREME_FEAR:  # <= 20
            base_frac *= 0.50  # v30: 50% size in extreme fear
        elif rsi4h_btc is not None and fgi_v is not None and rsi4h_btc < 40 and fgi_v > FGI_EXTREME_FEAR:
...
def current_position_qty(symbol: str) -> float:
    ...
    try:
        ...
        db_buy = float(cur.fetchone()[0] or 0.0)
        cur.execute("SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE symbol=? AND side='sell_partial' AND status='open'", (symbol,))
        db_partial = float(cur.fetchone()[0] or 0.0)
        conn.close()
    except Exception:
        pass

    db_net = max(0.0, db_buy - db_partial)

    # Exchange sanity cap (avoid overestimating position after partial sells).
    exch_total = None
    try:
        base = symbol.split('/')[0]
        bal = exchange.fetch_balance() or {}
        exch_total = float((bal.get('total', {}) or {}).get(base, 0.0) or 0.0)
    except Exception:
        exch_total = None
    ...
    if exch_total is None:
        return float(db_net)
    if db_net <= 0:
        return float(exch_total)
    return float(max(0.0, min(db_net, exch_total)))
...
def close_all_opens(reason: str = "equity_guard") -> dict:
    """
    Emergency close all open positions.
    Iterates _open_buys_all and closes each position.
    """
    result = {"attempted": 0, "closed": 0, "errors": 0, "remaining": 0}
    try:
        opens = _open_buys_all()
        result["attempted"] = len(opens)
        ...
        try:
            result["remaining"] = len(_open_buys_all())
        except Exception:
            result["remaining"] = -1
    except Exception as e:
        result["errors"] += 1
...
def _equity_guard_day_start(now_utc: datetime) -> datetime:
    day_start = now_utc.replace(hour=EQUITY_GUARD_RESET_HOUR_UTC, minute=0, second=0, microsecond=0)
    if now_utc < day_start:
        day_start -= timedelta(days=1)
    return day_start

def _equity_guard_state_get(key: str) -> str | None:
    try:
        conn = sqlite3.connect(DB_NAME); cur = conn.cursor()
        cur.execute("SELECT value FROM equity_guard_state WHERE key=? LIMIT 1", (key,))
        row = cur.fetchone()
        conn.close()
        return str(row[0]) if row else None
    except Exception as e:
        logger.debug(f"_equity_guard_state_get error ({key}): {e}")
        return None

def _equity_guard_state_set(key: str, value: str) -> None:
    try:
        now_iso = datetime.now(timezone.utc).isoformat()
        conn = sqlite3.connect(DB_NAME); cur = conn.cursor()
        cur.execute("""
            INSERT INTO equity_guard_state (key, value, updated_at)
            VALUES (?, ?, ?)
            ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at
        """, (key, str(value), now_iso))
        conn.commit(); conn.close()
    except Exception as e:
        logger.warning(f"_equity_guard_state_set error ({key}): {e}")

def _get_or_set_daily_start_equity(now_utc: datetime, current_equity: float) -> float | None:
    """
    Persist and load daily equity snapshot for true day-PnL calculations.
    Snapshot rolls at EQUITY_GUARD_RESET_HOUR_UTC.
    """
    try:
        day_start_dt = _equity_guard_day_start(now_utc)
        day_key = day_start_dt.date().isoformat()
        saved_day = _equity_guard_state_get("daily_start_day")
        saved_equity = _equity_guard_state_get("daily_start_equity")
        if saved_day != day_key or saved_equity is None:
            _equity_guard_state_set("daily_start_day", day_key)
            _equity_guard_state_set("daily_start_equity", f"{float(current_equity):.10f}")
            _equity_guard_state_set("daily_start_ts", day_start_dt.isoformat())
            logger.info(f"Equity guard daily snapshot set: day={day_key} equity={current_equity:.2f}")
            return float(current_equity)
        return float(saved_equity)
    except Exception as e:
        logger.warning(f"_get_or_set_daily_start_equity error: {e}")
        return None
...
def calculate_daily_pnl() -> tuple[float, float, float]:
    ...
    try:
        ...
        # True day-PnL from equity snapshot (avoids carrying historical unrealized losses across days)
        total_equity = get_total_equity_usdt()
        if total_equity is None:
            return realized_pnl, 0.0, 0.0

        day_start_equity = _get_or_set_daily_start_equity(now_utc, total_equity)
        if day_start_equity is None or day_start_equity <= 0:
            return realized_pnl, 0.0, 0.0

        day_total_pnl = float(total_equity - day_start_equity)
        unrealized_pnl = float(day_total_pnl - realized_pnl)
        total_pnl_pct = (day_total_pnl / day_start_equity * 100.0) if day_start_equity > 0 else 0.0
        return realized_pnl, unrealized_pnl, total_pnl_pct
    except Exception as e:
        logger.warning(f"calculate_daily_pnl error: {e}")
        return 0.0, 0.0, 0.0

def calculate_monthly_pnl() -> tuple[float, float]:
    """
    Calculate monthly PnL for monthly DD guard.
    Returns: (total_pnl_pct, starting_balance)
    """
    try:
        now_utc = datetime.now(timezone.utc)
        ...
        unrealized_pnl = 0.0
        for symbol, entry_price, amount in open_trades:
            try:
...
def check_equity_guard() -> tuple[bool, str]:
    ...
    try:
        ...
        if _EQUITY_MONTHLY_BREAKER_UNTIL is not None and today_date < _EQUITY_MONTHLY_BREAKER_UNTIL:
            if _EQUITY_MONTHLY_BREAKER_ACTIVE:
        ...
        if not _EQUITY_MONTHLY_BREAKER_ACTIVE:
            ...
            if monthly_start_bal is not None and monthly_pnl_pct <= EQUITY_GUARD_MONTHLY_MAX_DD_PCT:
                ...
                send_telegram_message(
                    f"🚨 *MONTHLY EQUITY GUARD TRIGGERED*\n"
                    f"Monthly PnL: `{monthly_pnl_pct:.2f}%`\n"
                    f"Threshold: `{EQUITY_GUARD_MONTHLY_MAX_DD_PCT}%`\n"
        ...
        # Get FGI to adjust threshold (more risk in fear = +EV)
        try:
            fgi_features = fetch_fgi_features()
            fgi_value = fgi_features.get("value")
        except Exception:
        ...
        threshold_total = EQUITY_GUARD_MAX_LOSS_PCT
        if fgi_value is not None and fgi_value <= 20:
            max_loss_threshold = EQUITY_GUARD_MAX_LOSS_PCT_FEAR
            threshold_total = EQUITY_GUARD_MAX_LOSS_PCT_FEAR
        ...
        if total_pnl_pct <= threshold_total:
            ...
            if should_close_all:
                close_result = close_all_opens(reason="equity_total_guard")
            logger.error(
            ...
            close_line = (
                f"Close-all: attempted={close_result.get('attempted', 0)} closed={close_result.get('closed', 0)} "
                f"errors={close_result.get('errors', 0)} remaining={close_result.get('remaining', 0)}\n"
                if close_result is not None else
                "Close-all: skipped (daily guard blocks new entries only)\n"
            )
            send_telegram_message(
                ...
                f"Emergency close threshold: `{EQUITY_GUARD_EMERGENCY_CLOSE_PCT}%`\n"
                f"Realized: `{realized_pnl:.2f}` USDT\n"
...
def trade_once_with_report(symbol: str):
    ...
    try:
        ...
        # 2a) v31: Daily trend filter — no new longs if BTC daily trend is bearish
        try:
            btc_lt = longterm_snapshot("BTC/USDT")
            _lt_px = btc_lt.get("last") if btc_lt else None
            _lt_ema = btc_lt.get("EMA200") if btc_lt else None
            if _lt_px is not None and _lt_ema is not None and _lt_px < _lt_ema:
                report["note"] = f"v31_daily_bear_block: BTC {_lt_px:.0f} < EMA200 {_lt_ema:.0f}"
                logger.info(f"{symbol}: {report['note']}")
                log_decision(symbol, 0.0, "hold", False)
                return report
        except Exception as e:
            logger.debug(f"v31 daily trend check error: {e}")

        # 2b) v31: Hard block in extreme panic — no trading when FGI <= 15
        try:
            _fgi_block = fetch_fgi_features()
            _fgi_val = _fgi_block.get("value") if _fgi_block else None
            if _fgi_val is not None and _fgi_val <= 15:
                report["note"] = f"v31_panic_block: FGI={_fgi_val}<=15"
                logger.info(f"{symbol}: {report['note']}")
                log_decision(symbol, 0.0, "hold", False)
                return report
        except Exception:
        ...
        # 2c) >>> EQUITY GUARD / CIRCUIT BREAKER (ANCHOR EQGUARD)
        ...
        # 4) v31: Fixed max trades limit (no dynamic boost)
        open_trades = get_open_trades_count()
        if open_trades >= MAX_OPEN_TRADES:
            report["note"] = f"max open trades ({MAX_OPEN_TRADES})"
        ...
        if mode == 'dca':
            try:
                df1h_dca = fetch_and_prepare_data_hybrid(symbol, timeframe="1h", limit=120)
                if df1h_dca is not None and len(df1h_dca) >= 50:
        ...
        # >>> SHORT MODE: Execute short if conditions met (WITH RISK CONTROLS)
            ...
            try:
                ...
                short_amount = get_short_amount(symbol, price_short, atr_abs_short)
        ...
        # >>> SWING MODE: Normal swing entry (default)
        # Use hybrid_decision for swing entries
        ...
        with buy_lock:
            ...
            # v31: Fixed max trades (no dynamic boost)
            open_trades = get_open_trades_count()
            if open_trades >= MAX_OPEN_TRADES:
                report["note"] = f"max open trades ({MAX_OPEN_TRADES}, post-lock)"
                log_decision(symbol, score, "hold", False)
                return report
            ...
            try:
                ...
                logger.error(f"Balance fetch error (post-lock) {symbol}: {e}")
                report["note"] = f"balance_error: {e}"
                log_decision(symbol, score, "hold", False)
                return report
...
def reconcile_orphan_open_buys():
    ...
    try:
        ...
        # v30-fix: SELL untracked positions instead of just alerting
        untracked = 0
        sold_orphans = 0
        for asset, amt in total.items():
            ...
            # v30: Actually sell the orphaned position
            try:
                free_amt = float(free.get(asset, 0) or 0.0)
                if free_amt <= 0:
                    logger.warning(f"[reconcile] Orphan {symbol} qty={float(amt):.6f} but free=0, skipping sell")
                    continue
                sell_amt, reason = _prepare_sell_amount(symbol, free_amt, px)
                if sell_amt <= 0:
                    logger.warning(f"[reconcile] Orphan {symbol}: {reason}")
                    continue
                order = exchange.create_market_sell_order(symbol, sell_amt)
                filled = float(order.get("filled", sell_amt) or sell_amt)
                sell_price = float(order.get("price", px) or px or 0.0)
                sold_orphans += 1
                logger.info(f"[reconcile] SOLD orphan {symbol}: qty={filled:.6f} @ {sell_price:.4f} (~${filled * sell_price:.2f})")
                send_telegram_message(
                    f"🧹 *Reconcile v30*: Vendido orphan `{symbol}` qty=`{filled:.6f}` @ `{sell_price}` (~${filled * sell_price:.2f})"
                )
            except Exception as e_sell:
                logger.error(f"[reconcile] Failed to sell orphan {symbol}: {e_sell}")
                send_telegram_message(f"⚠️ *Reconcile*: No pude vender orphan `{symbol}` qty=`{float(amt):.6f}`: {e_sell}")

        conn.close()
        total_issues = closed + untracked
        if total_issues:
            logger.info(f"[reconcile] Done: {closed} DB orphans closed, {sold_orphans}/{untracked} exchange orphans sold")
        else:
...
def startup_cleanup():
    ...
    try:
        ...
        for asset, amt in non_usdt.items():
            ...
            # v30-fix: Use actual filled amount from exchange, not requested amount
            sell_amt = float(order.get("filled", sell_amt) or sell_amt)
...
# Main loop (r4.2)
# =========================
# >>> DAILY MONITORING (Light Telegram daily, better than Freqtrade's heavy UI)
def get_daily_trades() -> list:
    """
    Get daily trades from database for reporting.
    """
    try:
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        ...
        cur.execute("""
            SELECT symbol, side, price, amount, ts, trade_id
            FROM transactions
            WHERE DATE(ts) = DATE(?) AND status='closed'
            ORDER BY ts
        """, (today.isoformat(),))
        rows = cur.fetchall()
        conn.close()
        ...
        trades = []
        for row in rows:
            symbol, side, price, amount, ts, trade_id = row
            # Calculate PnL (simplified: need to match buy/sell pairs)
            trades.append({
                'symbol': symbol,
                'side': side,
                'price': float(price),
        ...
        logger.error(f"get_daily_trades error: {e}")
        return []

def daily_report():
    """
    Send daily PnL report via Telegram (light monitoring, better than Freqtrade UI).
    """
    try:
        ...
        # Calculate PnL from trades (simplified: match buy/sell pairs)
        pnl_total = 0.0
        buy_trades = {}
        
        for trade in trades:
            if trade['side'] == 'buy':
                buy_trades[trade['trade_id']] = trade
            elif trade['side'] == 'sell' and trade['trade_id'] in buy_trades:
                buy = buy_trades[trade['trade_id']]
                pnl = (trade['price'] - buy['price']) * min(trade['amount'], buy['amount'])
                pnl_total += pnl
        
        # Get unrealized PnL from open positions
        unrealized_pnl = 0.0
        try:
            open_trades = _open_buys_all()
        ...
        total_pnl = pnl_total + unrealized_pnl
        total_pnl_pct = (total_pnl / 10000.0) * 100.0 if total_pnl else 0.0  # Assuming 10k starting balance
        
        message = (
            f"📊 *Daily Report*\n"
            f"Date: {datetime.now(timezone.utc).date()}\n"
            f"Trades: {len(trades)}\n"
            f"Realized PnL: {pnl_total:.2f} USDT\n"
            f"Unrealized PnL: {unrealized_pnl:.2f} USDT\n"
            f"Total PnL: {total_pnl:.2f} USDT ({total_pnl_pct:.2f}%)"
        )
        
        send_telegram_message(message)
        logger.info(f"Daily report sent: PnL={total_pnl:.2f} USDT ({total_pnl_pct:.2f}%)")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        # >>> BACKTEST MODE (Simple, faster than Freqtrade CLI)
        if BACKTEST_ENABLED:
            # Skip startup_cleanup in backtest mode (no API needed)
            logger.info("Backtest mode: skipping startup_cleanup and API initialization")
        else:
            try: exchange.load_markets()
        ...
        if BACKTEST_ENABLED:
            ...
            if OPTIMIZE_ENABLED:
                ...
                # Train ML model on backtest results if enabled
                if ML_ENABLED and SKLEARN_AVAILABLE and results.get('trades'):
                    # ML_MODEL is module-level, assign directly
                    globals()['ML_MODEL'] = train_ml_model(results['trades'])
                    logger.info("ML model trained on backtest results")
        ...
        while True:
            ...
            get_btc_rsi4h_cached()  # Ensure cache is fresh for this cycle
            ...
            try:
                # Check RSI4h for BTC (market proxy) - use cached helper
                rsi4h_btc = get_btc_rsi4h_cached()
            ...
            # Optional FGI gate (only if FORCE_FGI=1)
            if FORCE_FGI:
                try:
                    mk_bull, fgi_v, tag = is_market_bullish()
