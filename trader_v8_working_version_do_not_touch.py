    ...
    def loop():
        ...
        try:
            ...
            while True:
                ...
                # >>> PATCH B1: Volume-collapse exit (15m RVOL < threshold after min hold)
                if RVOL_COLLAPSE_EXIT_ENABLED:
                    try:
                        # Use the df15 we already computed above; if missing, fetch quickly
                        if 'df15' not in locals() or df15 is None or len(df15) < 20:
                            df15 = fetch_and_prepare_data_hybrid(symbol, timeframe="15m", limit=60)
                        rvol15_now = None
                        if df15 is not None and len(df15) > 20:
                            rv_mean_15 = df15['volume'].rolling(10).mean().iloc[-1]
                            if rv_mean_15 and rv_mean_15 > RVOL_MEAN_MIN:
                                rvol15_now = float(df15['volume'].iloc[-1] / rv_mean_15)
                        if held_long_enough and (rvol15_now is not None) and (rvol15_now < RVOL_COLLAPSE_EXIT):
                            logger.info(f"[collapse-exit] {symbol} 15m RVOL={rvol15_now:.2f} < {RVOL_COLLAPSE_EXIT:.2f} — exiting.")
                            sell_symbol(symbol, amount, trade_id, source="trail")
                            return
                    except Exception as e:
