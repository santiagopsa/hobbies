...
def calculate_established_strategy(indicators, data=None, symbol=None, coin_category_overrides=None):
    """
    New scoring system implementation per scalping spec:
      - Total scale 0-100.
      - Weights: MACD 25, EMA 20, RelVol 25, RSI 15, ADX 15, BB 10, Sentiment 5
      - Buy thresholds: stable >=80, volatile >=70
    Expects indicators dict to include macd/ema/volume/rsi/adx/bb info.
    """
    WEIGHTS = {
        'MACD': 25,
        'EMA': 20,
    ...
    norm = 100.0 / float(total_raw)

    rsi = indicators.get('rsi', None)
    rel_vol = indicators.get('relative_volume', None)
    adx = indicators.get('adx', None)
    macd_hist = indicators.get('macd_hist', None)
    has_macd_cross = indicators.get('has_macd_crossover', False)
    ema9 = indicators.get('ema9', None)
    ema21 = indicators.get('ema21', None)
    ema9_slope = indicators.get('ema9_slope', None)
    bb_lower = indicators.get('bb_lower', None)
    bb_width_pct = indicators.get('bb_width_pct', None)
    sentiment_score = indicators.get('sentiment_score', None) or indicators.get('market_sentiment', None) or 50
    short_vol_trend = indicators.get('short_volume_trend', 'insufficient_data')
    ...
    MIN_ADX = coin_category_overrides.get('MIN_ADX', 30) if coin_category_overrides else 30
    MIN_RELVOL = coin_category_overrides.get('MIN_RELATIVE_VOLUME', 1.8) if coin_category_overrides else 1.8

    if adx is None or adx < MIN_ADX:
        return "mantener", 40, f"ADX too weak ({adx}) < {MIN_ADX}"
    if rel_vol is None or rel_vol < MIN_RELVOL:
        return "mantener", 40, f"Relative volume too low ({rel_vol}) < {MIN_RELVOL}"
    if short_vol_trend != "increasing":
        return "mantener", 45, "Short volume trend not increasing"

    raw_score = 0.0

    # Rel Vol
    rel_vol_points = WEIGHTS['REL_VOL'] if rel_vol and rel_vol >= MIN_RELVOL else (WEIGHTS['REL_VOL'] * 0.5 if rel_vol and rel_vol >= (MIN_RELVOL * 0.8) else 0)
    raw_score += rel_vol_points
    ...
    if has_macd_cross:
        ...
        macd_ok = True
        raw_score += WEIGHTS['MACD'] * 0.8

    # EMA
    ema_ok = False
    if ema9 is not None and ema21 is not None and ema9 > ema21:
        slope_ok = (ema9_slope is not None and ema9_slope > 0)
        if slope_ok:
            ema_ok = True
            raw_score += WEIGHTS['EMA']
        else:
            raw_score += WEIGHTS['EMA'] * 0.6
    ...
    if rsi is not None:
        if rsi < 32:
            ...
            raw_score += WEIGHTS['RSI'] * 0.8
        elif rsi < 48:
            raw_score += WEIGHTS['RSI'] * 0.3

    # ADX
    if adx is not None and adx >= MIN_ADX:
        raw_score += WEIGHTS['ADX']
        if adx >= 40:
            raw_score += WEIGHTS['ADX'] * 0.3

    # BB
    if current_price is not None and bb_lower is not None:
        if current_price <= bb_lower:
            raw_score += WEIGHTS['BB']
        elif bb_width_pct is not None and bb_width_pct < 0.7:
            raw_score += WEIGHTS['BB'] * 0.7

    # Sentiment
    if sentiment_score is not None and sentiment_score > 65:
        raw_score += WEIGHTS['SENTIMENT']
    ...
    stable_coins = {'BTC', 'ETH'}
    buy_threshold = 80.0 if base_coin and base_coin.upper() in stable_coins else 70.0

    if score >= buy_threshold:
        if base_coin and base_coin.upper() in stable_coins:
            if not (macd_ok or ema_ok):
                return "mantener", int(min(75, score)), f"Score {score:.1f} but missing MACD/EMA confirmation for stable {base_coin}"
        action = "comprar"
        confidence = int(min(95, 50 + (score - buy_threshold) * (50.0/(100.0-buy_threshold))))
        explanation = f"Score {score:.1f} >= {buy_threshold}. ADX={adx}, RelVol={rel_vol}, RSI={rsi}, MACD_ok={macd_ok}, EMA_ok={ema_ok}"
    else:
        action = "mantener"
        confidence = 40 if score < 50 else 60
        explanation = f"Score {score:.1f} < {buy_threshold}. ADX={adx}, RelVol={rel_vol}, RSI={rsi}"
